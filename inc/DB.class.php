<?php
  ...
  class DB
  {
    ...
    /** Connect to a MySQL database to be able to use the methods below.
      */
    function DB($server = DBSERVER, $user = DBUSERNAME, $pass = DBPASSWORD, $base = DBDATABASE)
    {
    ...
    function resultToArray(){ 
       for ($count=0; $row = @mysql_fetch_assoc($this->lastResult); $count++)
         $res_array[$count] = $row;
    ...
    function resultToSingleArray(){
    	$this->lastResultArray = @mysql_fetch_assoc($this->lastResult);
    	return $this->lastResultArray;
    }
